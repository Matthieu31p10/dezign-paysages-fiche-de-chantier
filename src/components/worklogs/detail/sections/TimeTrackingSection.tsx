...
const TimeTrackingSection: React.FC = () => {
  ...
  // Handle the case where workLog might be null
  const departure = workLog?.timeTracking?.departure || '--:--';
  const totalHours = workLog?.timeTracking?.totalHours || 0;
  const calculatedEndTime = departure && totalHours ? calculateEndTime(departure, totalHours) : '--:--';
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="space-y-1">
          ...
          <p>{workLog?.timeTracking?.end || calculatedEndTime}</p>
        </div>
